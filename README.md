...
**What this is**: a free, open-source, cross-platform interpreter for GML 1.4, everyone's favourite game development language and toolkit. GMLarcana is based on Maiple's OpenGML and seeks to extend functionality to be binary compatible with GML 1.4 (but not 2.0) and also adds some known tricks from c++. **It's easy**: `gmla ./MyGame.project.gmx` in any terminal and your game will launch.

**What this is not**: an IDE or graphical user interface. If you want to GMLarcana for development, you must write your code, draw your sprites, and tile your rooms with your own preferred software. Recommendations are provided in the "[Other Software](#Other%20Software)" section below.
...
- **Debug**: Try `gmla --debug MyGame.project.gmx`
- **Build**: To compile GMLarcana from source yourself, see the [build instructions](./etc/BUILD.md).
- **Link**: To use GMLarcana as a C++ library for building your own tool, see the [Library guide](./etc/LIBRARY.md).
- **Develop**: If you are interested in helping to develop GMLarcana, or you'd like to modify or hack it, or you just want to get an overview of the codebase, see both the [build instructions](./etc/BUILD.md) and the [Source Code Overview](./etc/SOURCE_OVERVIEW.md).
- **Recurse**: To use GMLarcana from within GML (e.g. to replace `string_execute()`), see the usage instructions for [gig](./src/gig/README.md).
...
- Available as a **stand-alone binary** to interpret GML: `gmla ./MyGame.project.gmx`
- Can also be used as a **C++ library** to parse, compile, and execute GML code: `#include <ogm/ast/parse.h>`
- Comes with a **debugger**. Place conditional breakpoints and watchpoints, view and edit variables in place, and step through source code or bytecode instructions if you prefer. Available from the command-line with `gmla --debug MyGame.project.gmx`.
...
- C-Tricks (as i like to call it), uint64 support, structs, functions

## Other Software

Here are some software suggestions which may be helpful for creating games with GMLarcana. Not all of these have been tested for interoperability.
...
If you decide you don't like GMLarcana, you may be interested in these alternatives. Not all of them are complete or usable.
